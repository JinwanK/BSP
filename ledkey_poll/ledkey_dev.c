...
static irqreturn_t key_isr(int irq, void *data) // void 포인터는 형 변환해서 사용
{
	int i;
	keyData * pKeyData = (keyData *)data;  // 36바이트에 36바이트 형변환 해서 저장
...
static int gpioKeyIrqInit(keyData * pKeyData)
{
	...
	for(i=0;i<GPIOKEYCNT;i++)
	{
		...
		if(pKeyData->key_irq[i] < 0)
		{
			...
			return pKeyData->key_irq[i]; //오류를 리턴
		}
	}
	for(i=0;i<GPIOKEYCNT;i++)
	{
...
static int ledkey_open (struct inode *inode, struct file *filp)
{
	...
	//좌측이 크고 우측이 작으면 자동으로 형 변환. 지금은 크니까 형 변환으로 작게 만들었다.
	if(!pKeyData)
		return -ENOMEM;
...
//			wait_event_interruptible_timeout(WaitQueue_Read, pKeyData->keyNumber, 100);
			//100 * 1/HZ = 100 * 1/100 = 100 * 0.01 = 1Sec
		}
	}
